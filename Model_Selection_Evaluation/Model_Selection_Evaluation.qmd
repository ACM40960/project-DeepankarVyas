...
# Initializing empty lists to store results for each dataset
...
# Loading other models for Class A
...
# Loading other models for Class A NLP
...
# Combining split SVM model parts for Class B
...
# Loading other models for Class B
...
# Combining split SVM model parts for Class B NLP
...
# Loading other models for Class B NLP
...
```{r}
# Setting up the output directory for saving plots and results
  ...
  if (!individual) {
    ...
    ggplot(data_plot, aes(x = Model, y = Mean_val, fill = Model)) +
      ...
      labs(title = paste(metric_name, "Across All Models"), 
           x = "Model", 
           y = metric_name) +
      scale_fill_brewer(palette = "Paired") +  # Using a smoother color palette
      theme_minimal(base_size = 12) +
      theme(
        axis.text.x = element_text(angle = 45, hjust = 1),
        plot.title = element_text(hjust = 0.5, size = 14, face = "bold"),
        legend.position = "none",
        plot.title.position = "plot"  # Moving title outside of the plot area
      ) +
      facet_wrap(~ Dataset)
  } else {
    # Filtering the dataset for the draw class
    draw_class_data <- metrics_combined %>% filter(Class == "Class: D")
    
    # Creating the bar plot for the draw class
    ggplot(draw_class_data, aes(x = Model, y = !!sym(metric_name), fill = Dataset)) +
      ...
      labs(title = paste(metric_name, "for Draw Class Across Models and Datasets"),
           x = "Model",
           y = metric_name) +
      scale_fill_brewer(palette = "Set3") +  # Using a smoother color palette
      theme_minimal(base_size = 12) +
      theme(
...
# Defining a function to combine metrics from all models
combine_metrics <- function(results_list, dataset_name) {
  # Extracting metrics and combining them into a data frame
...
# Defining a function to evaluate and plot metrics
evaluate_and_plot <- function(metrics_combined) {
  
  # Plotting metrics across all classes
  plot_all_classes_metrics(metrics_combined, "mean_balanced_accuracy")
  ggsave(file.path(output_dir, "Plot_mean_balanced_accuracy.pdf"))
  
  plot_all_classes_metrics(metrics_combined, "mean_f1_score")
  ggsave(file.path(output_dir, "Plot_mean_f1_score.pdf"))
  
  plot_all_classes_metrics(metrics_combined, "mean_rps")
  ggsave(file.path(output_dir, "Plot_mean_rps.pdf"))
  
  plot_all_classes_metrics(metrics_combined, "mean_f1_score", individual = TRUE)
  ggsave(file.path(output_dir, "Plot_mean_f1_draw.pdf"))
  
  plot_all_classes_metrics(metrics_combined, "mean_sensitivity", individual = TRUE)
  ggsave(file.path(output_dir, "Plot_mean_sensitivity_draw.pdf"))
  
  plot_all_classes_metrics(metrics_combined, "mean_precision", individual = TRUE)
  ggsave(file.path(output_dir, "Plot_mean_precision_draw.pdf"))
  
  return(metrics_combined)
}

# Defining a function to prepare data for multiROC
prepare_multiroc_data <- function(results_list) {
  multi_roc_data <- list()
  
  # Iterating through each dataset and model
...
# Defining a function to plot ROC curves for each model of each dataset and for each class of the model
plot_multiroc_curves <- function(multi_roc_data) {
  plot_list <- list()
  
  # Iterating through each ROC data set
  for (roc_data_name in names(multi_roc_data)) {
    ...
    # Updating Group labels with AUROC values
...
# Defining a function to build and compile a Keras model
...
# Defining a function to calculate metrics for each class and return them
...
# Defining a function to retrain the best model and evaluate on the test set
retrain_and_evaluate <- function(best_model, best_dataset, train_val_data, test_data) {
  
  if (best_model == "nn") {
    # Retrieving optimal dropout and lambda values from stored values
    ...
    # Directly using the existing model for other types of models
    ...
    # Plotting the ROC curves with updated legend labels
    ggplot(roc_res_df, aes(x = 1 - Specificity, y = Sensitivity, color = Group)) +
      geom_line() +
      geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
      labs(
...
# Initializing a list to store results for each dataset
results_list <- list(
...
# Looping through each dataset in the results list
for (dataset in names(results_list)) {
  # Checking if the dataset has the neural network model
  if ("nn" %in% names(results_list[[dataset]])) {
    # Updating the mean_rps to rps
...
# Combining metrics from all datasets
...
# Evaluating and plotting the combined metrics
metrics_combined <- evaluate_and_plot(metrics_combined)

# Calculating the means for each dataset
...
# Calculating the mean_f1_score for the draw class
draw_class_means <- metrics_combined %>%
  filter(Class == "Class: D") %>%
  arrange(Dataset, Model)

# Merging the draw class means back to the original dataframe
ranking_dataset <- cbind(dataset_means, draw_class_means$mean_f1_score)

# Changing the column name for the draw class mean F1 score
colnames(ranking_dataset)[ncol(ranking_dataset)] <- "mean_f1_draw"

ranking_dataset <- ranking_dataset %>% filter(!is.na(mean_f1_draw))

# Heuristic to rank models
# Step 1: Ranking based on the highest mean_f1_draw
ranking_dataset <- ranking_dataset %>%
  arrange(desc(mean_f1_draw))

# Step 2: Exploring different threshold values
highest_f1_draw <- ranking_dataset$mean_f1_draw[1]
threshold_values <- seq(0.01, 0.1, by = 0.01)

# Defining a minimum threshold to ensure at least 2 models
minimum_threshold <- 0.05

# Defining a function to count the number of models within a given threshold
...
# Applying the function to all threshold values
model_counts <- sapply(threshold_values, count_models_within_threshold)

# Plotting the number of models within each threshold
...
# Identifying the best model and dataset based on the ranking
best_model <- final_ranking_dataset$Model[1]
best_dataset <- final_ranking_dataset$Dataset[1]

# Retraining the best model and evaluating on the test set
train_val_data <- read.csv(paste0("../Dataset/", best_dataset, "_train.csv"))  # Combining training and validation data
test_data <- read.csv(paste0("../Dataset/", best_dataset, "_test.csv"))

# Retrieving the final evaluation metrics
final_eval_metrics <- eval(parse(text = paste0("results_list[['", 
                                               best_dataset, "']][['",
                                               best_model, "']]$metrics")))

# Saving the final evaluation metrics to a PDF file
...
# Retraining the best model and evaluating it on the test set
...
# Saving the best model metrics to a PDF file
...
# Computing implied probabilities based on betting odds
...
# Computing the sum of implied probabilities
booksum <- rowSums(implied_probs)

# Normalizing the probabilities to ensure they sum to 1
normalized_probs <- implied_probs / booksum

# Calculating the Ranked Probability Score (RPS)
...
# Calculating the mean RPS for BET365
mean_rps_bet365 <- customSummary(data, lev = c('A', 'D', 'H'))

# Fixed value for comparison of baseline paper
fixed_value <- 0.2156

# Retrieving the RPS of the final model
best_model_rps <- best_model_metrics$rps[1]

# Creating a dataframe for RPS comparison
...
# Defining the colors for each model in the bar plot
colors <- c("BET365" = "#FF9999", "Baboota_Kaur" = "#66B2FF")
colors[paste0(best_model, "_", best_dataset)] <- "#99FF99"

# Plotting the barplot for RPS comparison
...
# Saving the RPS comparison plot to a PDF file
ggsave(file.path(output_dir, "rps_comparison_plot.pdf"))
